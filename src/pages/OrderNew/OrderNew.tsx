...
import { useAuth, useCurrentUserId, useCurrentUserName } from '../../hooks/useAuth';
import {
    appendOrderHistory,
    createAttachment,
    createLineCut,
...
function emptyLine(): LineDraft {
    return { id: newLineId(), productId: '', freeText: '', qty: '1', unit: 'pcs', price: '', cuts: [] };
}
...
export function OrderNew() {
    ...
    const { prefill, rawText, attachments } = (location.state as NewOrderLocationState) || {};
    ...
    useEffect(() => {
        ...
        async function loadOptions() {
            ...
            const [c, p] = await Promise.all([
                ...
                readProducts({ fields: ['id', 'name', 'catch_weight', 'oos'], filter: { oos: { _neq: true } }, limit: -1, sort: ['name'] }),
            ]);
            if (cancelled) return;
    ...
    useEffect(() => {
        ...
        async function refresh() {
            const res = await readOrders({ fields: ['no'], filter: { no: { _starts_with: dc } }, limit: -1 });
            if (cancelled) return;
            setExistingOrderNos((res.data ?? []).map((o) => o.no).filter((n): n is string => !!n));
            if (!orderNoTouched) {
    ...
    const usedSeqs = new Set(
        existingOrderNos.map(parseOrderNo).filter((p): p is NonNullable<typeof p> => !!p && p.dateCode === dcOf).map((p) => p.seq)
    );
    let lowestFreeSeq = 1;
    ...
    const seqGap = !dupOrder && !badNo && dcOf && parsedEntered?.dateCode === dcOf && parsedEntered.seq !== lowestFreeSeq;
    ...
    function handleCustomerNameChange(name: string) {
        ...
        const exact = customers.find((c) => c.name.trim().toLowerCase() === name.trim().toLowerCase());
        if (exact) {
    ...
    async function handleSubmit(e: FormEvent) {
        e.preventDefault();
        if (!allowed) { setError("Your role doesn't have permission to create orders."); return; }
        if (!customerName.trim()) { setError('Enter a customer / restaurant name.'); return; }
        if (badNo) { setError('Order number format is invalid.'); return; }
        if (dupOrder) { setError(`Order #${orderNoValue} already exists.`); return; }
        ...
        cleanLines.forEach((l, i) => {
            ...
            l.cuts.forEach((c, ci) => {
                ...
                if (text) cutInputs.push({ line_id: savedLine.id, text, sort_order: ci });
            });
        });
        if (cutInputs.length > 0) {
            const cutResults = await Promise.allSettled(cutInputs.map((c) => createLineCut(c)));
            const failedCuts = cutResults.filter(
                (r) => r.status === 'rejected' || (r.status === 'fulfilled' && r.value.error !== null)
            ).length;
            if (failedCuts > 0) {
                window.alert(`Order created, but ${failedCuts} cutting instruction(s) failed to save. You can add them from the order page.`);
            }
        }
        ...
        if (attachments && attachments.length > 0) {
            const attachmentResults = await Promise.allSettled(
                attachments.map(async (file) => {
                    ...
                    if (uploadRes.error || !uploadRes.data) throw new Error(uploadRes.error ?? 'upload failed');
            ...
            const failedCount = attachmentResults.filter((r) => r.status === 'rejected').length;
            if (failedCount > 0) {
                window.alert(`Order created, but ${failedCount} attachment(s) failed to upload. You can add them from the order page.`);
            }
        }

        await appendOrderHistory({ order_id: orderId, what: 'Order created', who: null, stage: 'intake' });
    ...
    return (
        <div className={styles.container}>
            <div className={[styles.layoutGrid, isPanelOpen ? styles.layoutGridWithPanel : styles.layoutGridFull].join(' ')}>
                <div className={styles.mainColumn}>
                    <header className={styles.header}>
                        ...
                        <div className={styles.actions}>
                            ...
                            >
                                <Icon name="save" size={16} /> {submitting ? 'Creating…' : 'Create order'}
                            </Button>
                        </div>
                    </header>

                    {!allowed && <p className={styles.error} role="alert">Your role doesn't have permission to create orders.</p>}
                    {loadingOpts && <p className={styles.muted}>Loading customers + products…</p>}

                    <form id="new-order-form" className={styles.form} onSubmit={handleSubmit}>
                        {multiCustomer && (
                        ...
                        <Card className={styles.customerCard}>
                            ...
                            <div className={styles.row}>
                                <label className={styles.field}>
                                    ...
                                    <input
                                        ...
                                        value={orderNoValue}
                                        onChange={(e) => { setOrderNoValue(e.target.value.replace(/\D/g, '')); setOrderNoTouched(true); }}
                            ...
                            <div className={styles.row}>
                                <label className={styles.field}>
                                    ...
                                    <input
                                        type="text" className={styles.input} list="customer-suggestions"
                                        value={customerName} onChange={(e) => handleCustomerNameChange(e.target.value)}
                                        disabled={submitting || !allowed} placeholder="Type or select a customer" required
                                    />
                                ...
                                <label className={styles.field}>
                                    ...
                                    <input
                                        ...
                                        disabled={submitting || !allowed} placeholder="e.g. PT En Prima Food & Beverages"
                                    />
                                </label>
                            </div>
                            ...
                            {customerMatch.type === 'fuzzy' && customerMatch.customer && (
                                <div className={styles.matchBanner} data-tone="warning">
                                    Did you mean <b>{customerMatch.customer.name}</b>? Looks like the same customer.
                                    <button type="button" className={styles.addLineBtn} style={{ marginLeft: 8 }}
                            ...
                            {customerMatch.type === 'phone' && customerMatch.customer && (
                                <div className={styles.matchBanner} data-tone="success">
                                    Matched by phone number to <b>{customerMatch.customer.name}</b>.
                                    <button type="button" className={styles.addLineBtn} style={{ marginLeft: 8 }}
                                        onClick={() => acceptCustomerMatch(customerMatch.customer!)}>
                                        Use {customerMatch.customer.name}
                                    </button>
                            ...
                            <div className={styles.row}>
                                <label className={styles.field}>
                                    ...
                                    <input type="text" className={styles.input} value={customerPhone}
                                        onChange={(e) => setCustomerPhone(e.target.value)}
                                        disabled={submitting || !allowed} placeholder="WhatsApp / phone" />
                                </label>
                                <label className={styles.field}>
                                    <span className={styles.label}>Sales Rep</span>
                                    <input type="text" className={styles.input} value={sales}
                                        onChange={(e) => setSales(e.target.value)}
                                        disabled={submitting || !allowed} placeholder="Auto-filled from your name" />
                                </label>
                            </div>
                            <label className={styles.field}>
                                <span className={styles.label}>Delivery Address <span className={styles.caption}>(Optional)</span></span>
                                <textarea className={styles.input} style={{
                                    ...
                                    disabled={submitting || !allowed} placeholder="Delivery address" />
                            </label>

                            <label className={styles.field}>
                                <span className={styles.label}>Notes <span className={styles.caption}>(Optional)</span></span>
                                <textarea className={styles.input}
                                    ...
                                    disabled={submitting || !allowed} placeholder="Any note for this order..." />
                            </label>
                        </Card>

                        <Card>
                            <div className={styles.heading}>Items <span className={styles.count}>{lines.length}</span></div>
                                            ...
                                            <div className={styles.column}>
                                                <div className={styles.itemHeaderRow}>
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="Qty" disabled={submitting || !allowed} />
                                                    <select
                                                ...
                                                {!l.productId && (
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            setLines((prev) =>
                                                                prev.map((l) => (l.id === l.id ? { ...l, name: val } : l))
                                                            );
                                                        }}
                                                    />
                                                )}
                                            ...
                                            {l.cuts.map((cut) => (
                                                <div key={cut.id} className={styles.cutRow}>
                                                            ...
                                                            setLines((prev) =>
                                                                prev.map((ln) =>
                                                                    ln.id === l.id
                                                                        ? {
                                                                            ...ln,
                                                                            cuts: ln.cuts.map((c) => (c.id === cut.id ? { ...c, text: val } : c)),
                                                                        }
                                                                        : ln
                                                                )
                                                            );
                                                    ...
                                                    <Button
                                                        ...
                                                        size="sm"
                                                        iconOnly
                                                        onClick={() => handleDeleteCutFromLine(l.id, cut.id)}
                                                    >
                                                        <Icon name="trash" size={14} />
                                                    </Button>
                                                </div>
                                        ...
                                        <div className={styles.itemPriceRow}>
                                            ...
                                            <div className={styles.priceCalc}>
                                                ...
                                                <span className={styles.lineTotalPrice}>
                                                    {currency.format((parseFloat(l.price) || 0) * (parseFloat(l.qty) || 0))}
                                                </span>
                                            </div>
                                        </div>
                ...
                <aside className={styles.sidePanelColumn}>
                    <Button
                        type="button" variant="secondary" iconOnly
                    ...
                    <div className={[styles.sidePanelStickyContent, !isPanelOpen ? styles.sidePanelStickyContentCollapsed : ''].join(' ')}>
                        <Card className={styles.notesCard}>
                            <h3 className={styles.heading}>Original WhatsApp message</h3>
                            {rawText && rawText.trim() ? (
                                <pre className={styles.pre}>{rawText}</pre>
                            ) : (
                                <p className={styles.muted}>No pasted message — entered manually.</p>
                            )}
                            {attachments && attachments.length > 0 && (
                                <div className={styles.attachmentsNote}>
                                    <p className={styles.muted}>
                                        {attachments.length} attachment{attachments.length > 1 ? 's' : ''} will be added as PO document{attachments.length > 1 ? 's' : ''}:
                                    </p>
                                    <ul>{attachments.map((f, i) => <li key={i}>{f.name}</li>)}</ul>
                                </div>
                            )}
                        </Card>
                    </div>
