...
function getFilterForField(
  rangeVal: DateRangeVal,
  field: "order_date" | "delivered_at" | "created_at" | "updated_at",
): Record<string, unknown> {
  // 'all' = no date restriction at all — every record, all time.
  if (rangeVal.type === "all") return {};
  ...
  if (isDatetime) {
    ...
    if (
      rangeVal.type === "today" ||
      (rangeVal.type === "specific" && rangeVal.date)
    ) {
...
const EMPTY_METRICS: DashboardMetric[] = [
  { id: "open", label: "Open Orders", value: 0, range: "All time" },
...
export function useDashboardCounts(
  totalRange: RangeWithLabel = { val: { type: "all" }, label: "All" },
  deliveredRange: RangeWithLabel = { val: { type: "all" }, label: "All" },
  cancelledRange: RangeWithLabel = { val: { type: "all" }, label: "All" },
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const openFilter = openOrdersFilter();
      ...
      const financeExtra = financeRes.data
        ? extractCount(financeRes.data[0]?.count)
        : 0;
      if (financeExtra > 0) {
      ...
      stageMap.set(
        "awaiting_return",
        awaitingReturnRes.data
          ? extractCount(awaitingReturnRes.data[0]?.count)
          : 0,
      );
      stageMap.set(
        "admin_action",
      ...
      stageMap.set(
        "awaiting_signed_doc",
        awaitingSignedDocRes.data
          ? extractCount(awaitingSignedDocRes.data[0]?.count)
          : 0,
      );
      stageMap.set(
        "replacement_transit",
        replacementTransitRes.data
          ? extractCount(replacementTransitRes.data[0]?.count)
          : 0,
      );
      ...
      const totalVal = totalRes.data
        ? extractCount(totalRes.data[0]?.count)
        : 0;
      const deliveredVal = deliveredRes.data
        ? extractCount(deliveredRes.data[0]?.count)
        : 0;
      const cancelledVal = cancelledRes.data
        ? extractCount(cancelledRes.data[0]?.count)
        : 0;

      setMetrics([
        { id: "open", label: "Open Orders", value: openVal, range: "All time" },
        {
          id: "total",
          label: "Total Orders",
          value: totalVal,
          range: totalRange.label,
        },
        {
          id: "delivered",
          label: "Delivered Orders",
          value: deliveredVal,
          range: deliveredRange.label,
        },
        {
          id: "cancelled",
          label: "Cancelled Orders",
          value: cancelledVal,
          range: cancelledRange.label,
        },
      ]);
