...
export function AddItemModal({
  ...
  const [matchedItem, setMatchedItem] = useState<AddItemResult>(() =>
    defaultResult(products, unitOptions),
  );
  ...
  function parseFreeTextLine(
    ...
    if (!matchedProduct && searchName) {
      ...
      matchedProduct = productsList.find(
        (p) => p.name.toLowerCase() === sLower,
      );
      if (!matchedProduct) {
        matchedProduct = productsList.find(
          (p) =>
            p.name.toLowerCase().includes(sLower) ||
            sLower.includes(p.name.toLowerCase()),
        );
      }
      if (!matchedProduct) {
    ...
    >
      <div className={styles.addItemModalCard}>
        ...
        <div className={styles.subtitle}>
          {isManual
            ? t("or add manually:")
            : t("Matched result — review and adjust:")}
        ...
        <div className={styles.matchedResultRow}>
          <input
            ...
            step="0.5"
            onChange={(e) =>
              setMatchedItem((prev) => ({ ...prev, qty: e.target.value }))
            }
          />
          <select
            ...
            onChange={(e) =>
              setMatchedItem((prev) => ({ ...prev, unit: e.target.value }))
            }
          >
            {unitOptions.map((u) => (
        ...
        {!matchedItem.productId && (
          <input
            type="text"
            className={styles.editInput}
            placeholder={t("Item name (custom)")}
            value={matchedItem.name}
            onChange={(e) =>
              setMatchedItem((prev) => ({ ...prev, name: e.target.value }))
            }
          />
        )}
