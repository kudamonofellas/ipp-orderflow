...
## Update — 2026-09-08 `sectionsContainer` — the standard per-page content wrapper

- **Every routed page now nests its content in its own `<div className={styles.sectionsContainer}>`**, immediately inside the page's existing root element — `display:flex; flex-direction:column; gap:var(--space-xl); padding:0 var(--space-xl);`, added to that page's own `.module.css`. This is Dashboard's own pre-existing pattern (its `sectionsContainer` holding the digest/metrics/pipeline/panels/open-orders stack, kept separate from its full-bleed `topRow`), rolled out to `Orders`, `Customers`, `Products`, `CustomerDetail`, `ProductDetail`, `CustomerEdit`, `ProductEdit`, `CustomerNew`, `ProductNew`, `OrderNew`, `OrderEdit`, `OrderDetail`, `Deliveries`, `PickList`, `CashUp`, `LearnedMatches`, `Reports`, and `Settings`. Any *new* page should get this wrapper too, matching the exact class name and values — don't invent a differently-named or differently-valued wrapper for the same job. `Login` is the one exception (rendered outside `AppLayout`, no sidebar gutter to match).
- **Deliberately per-page, not a single shared wrapper in `AppLayout`** — considered and reverted: wrapping `<Outlet/>` once in `AppLayout.tsx` would apply padding globally, but it also traps a page's own full-bleed elements (Dashboard's `topRow`, whose border-bottom needs to span edge-to-edge past the gutter) inside the same padded box, forcing an awkward negative-margin escape hatch just to undo it. Keeping the wrapper page-local means a page can simply place a full-bleed element as a sibling outside its own `sectionsContainer` (exactly what Dashboard already does with `topRow`) with no extra CSS trick needed.
- **Loading/error early-return states get the wrapper too**, not just the main render path — a page that does `if (loading) return <div className={styles.container}>...</div>;` before its main `return (...)` needs that early return ALSO carrying the `sectionsContainer` nesting, or the loading text renders unpadded while everything else on the page is gutter-aligned.
- **Loading-state text is `<p className={styles.muted}>{t("Loading …")}</p>`, everywhere** — a plain, small `.muted` class (`font: var(--text-body); color: var(--text-muted); margin: 0;`) defined per-page (CSS Modules, no shared import), not a bespoke `.loading` class with its own padding/centering (Dashboard had one; replaced). Table-row loading states (a `<td>` spanning the table, e.g. Customers/Products lists) are the one exception — their existing `.stateRow` class already resolves to the identical color/font, just scoped correctly for a table cell, so leave those as `.stateRow`, don't force a `<p className={styles.muted}>` into a `<td>`.
- **A generic table (not already living inside a purpose-specific card) doesn't need a `<Card>` wrapper** — Customers' and Products' own tables dropped their `<Card>` in favor of a plain `.tableWrapper` div carrying the exact same visual treatment (`background: var(--bg-surface); border: 1px solid var(--border-default); border-radius: var(--radius-lg); overflow: hidden;`) directly in that page's own module — one fewer generic, reusable component in a render path that was never going to reuse `Card`'s other features (header, padding variants) anyway.
- **A section split off `Dashboard.tsx` into its own component belongs in `sections/`, with its own `.module.css`** — `PipelineRow`, `MetricsRow`, `DigestSection`, and `QuickActionsRow` (all split out 2026-09-08) follow the exact shape `AttentionPanel`/`OpenOrdersPanel`/`ReturnWorkflowsPanel` already established: presentational props in (pre-computed data + callbacks), no data-fetching hooks of its own, `useLanguage()` for its own `t()`. Apply this whenever a page's inline JSX for one visual section grows past a handful of lines — don't let a page component keep accreting inline sections indefinitely.
- **A 2-column panel row doesn't need one panel's height forced onto the other via a scrollable clamp** — Dashboard's `panelsGridTwo` used to lock its second column (`AttentionPanel`) to the first column's height with `height:0; min-height:100%; max-height:100%` plus an internal `overflow-y:auto` list, so a long "Needs Attention" list scrolled inside a fixed box instead of growing. Removed per direct request — `align-items:start` on the grid, no height clamp, each panel just takes its own natural content height. Reach for this (not a forced-height scroll box) any time two panels sit side-by-side and one's content length is unpredictable.

## Update — 2026-09-03 (2) `Thumbnails` gallery component removed — reverted to plain always-expanded thumbnails

- **The `Thumbnails` component described in the entry directly below is removed, not superseded** — reported directly as a real performance regression ("too slow when uploading attachments"), traced to its per-instance `ResizeObserver` + `useLayoutEffect`: cheap in isolation, but `OrderDetail.tsx` can carry 20+ live instances on one order (9 call sites, 2 inside the per-line `.map()`) with zero memoization anywhere in this codebase, so any single-line photo upload re-renders — and re-measures — every gallery on the page. Treat the entry below as historical record of a pattern that no longer exists; don't reach for "Thumbnails" or its collapse-to-modal/measured-overflow conventions as reusable shapes anymore.
- **Replacement: `renderThumbnails()`, a plain function nested inside `OrderDetail()`** (not a module-level component) — renders every photo, always, wrapping onto a new line via CSS `flex-wrap` instead of collapsing into an "N more" overlay. No observer, no layout effect, no per-instance measurement. Nested (not module-scope) specifically so it can close over `openImageGallery`/`deleteImageEntry` directly, rather than every call site passing them in — the "plain function called inline, not `<Foo/>`" pattern already documented above for `renderPanel()` (no remount risk, since it's invoked as `{renderThumbnails(...)}`, not rendered as a JSX element). This is the pattern to reach for now when a row needs to show N photos as small clickable thumbnails: a plain wrapping row, not a measured-collapse one — the overflow-collapse polish was a real usability nicety, but it's not worth reintroducing without first memoizing the rows that contain it (nothing in this codebase does today).
- **Per-thumbnail hover-trash delete restored, same turn** (initially dropped in the revert, then flagged missing and re-added) — `deleteImageEntry(entry)`/`canDeleteImageEntry(entry)`, two small component-scope helpers, resolve and fire the right removal handler purely from *which* id-fields an `ImageModalEntry` carries (only one group is ever populated per entry). Both `renderThumbnails()`'s hover-trash icon *and* the `<ImageDetailsModal>`'s own delete button now call the same two helpers — previously the modal had its own ~45-line inline branch chain duplicating this logic; now there's exactly one place that knows how to delete "whatever this entry is." `ReturnLineBox.tsx` got its `onRemovePhoto` prop back too (removed by the original `Thumbnails` PR as "now-dead"), wired to `handleRemoveReceiveWeighPhoto` — restored only on its *pending/interactive* photo row, not its *confirmed* read-only one, matching the pre-`Thumbnails` behavior exactly (you can delete evidence you're still weighing in, not evidence already locked into a confirmed record).
- **What stayed**: the `ImageDetailsModal` slideshow (`onPrev`/`onNext`/`currentIndex`/`total`, arrow-key/Escape nav) and its generic per-entry delete wiring (`activeImageModal.gallery`/`galleryIndex`) are completely untouched — `openImageGallery(photos, index)` is still called the same way from the new plain thumbnail rows, so multi-photo rows still open as a navigable, deletable slideshow. Only the *thumbnail row's own rendering* reverted; the modal-centric interaction model from the entry below stayed.

## Update — 2026-09-03 `Thumbnails` gallery component; measured (not assumed) overflow collapse

- **New `src/components/Thumbnails/`** — the standard way to render a list of photo thumbnails inline anywhere in the app, replacing the old "map every photo to a `.thumbnailItem` div" pattern that used to live inline at every call site (and would silently wrap onto a second line once it ran out of room, corrupting the surrounding layout — reported directly with a screenshot). Renders every photo individually, one thumbnail each, for as long as they fit on one line; only collapses to a single thumbnail + hover "N more" overlay once real overflow is detected. Use this any time a card/row needs to show N photos as small clickable thumbnails — don't hand-roll a `.map()` of thumbnail divs again.
- **Measure real overflow with `ResizeObserver`, don't assume a fixed collapse threshold** — `Thumbnails` keeps a full, un-collapsed row always laid out (`visibility: hidden`, not unmounted, when the collapsed view is what's actually shown) purely so its `scrollWidth` vs `clientWidth` stays comparable on every resize. This is the general pattern for "show inline until it doesn't fit, then collapse" — don't hardcode "collapse past N items," since the actual fit point depends on container width, sidebar-collapsed state, viewport size, and photo count all at once.
- **Collapse-into-modal supersedes per-thumbnail hover-delete** — every site that used to show a hover-triggered trash icon directly on each inline thumbnail now deletes via the `ImageDetailsModal`'s own trash button instead (open the photo, then delete it) — one consistent delete path everywhere, rather than two ways to do the same thing depending on whether a row happened to implement its own hover-trash. If a future photo-list feature needs delete, wire it through the modal's `onDelete`, not a new per-thumbnail overlay icon.
- **`ImageDetailsModal` slideshow**: gained `onPrev`/`onNext`/`currentIndex`/`total` props — a "N / M" counter in the header plus prev/next nav rendered as the standard icon-only `Button` (`chevronLeft`/`chevronRight`, translucent-circle inline style matching the existing close/delete buttons in the same header), only shown when `total > 1`. Arrow keys + Escape work while open. The modal itself stays agnostic about *why* there are multiple photos or what deleting one means — the parent (`OrderDetail.tsx`'s `activeImageModal.gallery`/`galleryIndex`) owns that, swapping the whole active-entry object (including its delete-target ids) on each page so the modal's existing single-image `onDelete` wiring works unmodified for a slideshow.
- **Two independent emptiness checks before rendering a card**: the Delivery Proof card now computes *both* "is there anything in this specific sub-section" (e.g. `deliveryDocPhotos.length > 0`) *and* "is there anything anywhere in this card" (used to gate the whole `<Card>`) as separate booleans, checked before the JSX return rather than only gating the outer card and assuming every inner section handles its own emptiness. A card with multiple independent optional sections needs both: an outer existence check (render the card at all?) and per-section checks (render this particular part?) — one doesn't imply the other.

## Update — 2026-09-02 Status banner vs. decision card as two separate `Card`s, not one branched card; new `pillSuccess`; nested bordered-pill list

- **A status banner and the decision card it can trigger are two separate `Card`s, not one card that swaps its own content** — `OrderDetail.tsx`'s outstanding-order view first tried branching a single card's content on `owedLines.length`, then was corrected: the plain "Outstanding" status banner (icon + one-paragraph explanation, unconditional for every `outstanding` order) stays exactly where it was in the stepper section; the new "Part delivered — the rest is still owed" decision card (owed-lines list + 3 action buttons, gated on `owedLines.length > 0`) is a second, independent `Card` positioned near the content it acts on (right below the Items card), not a replacement for the status banner. Reach for two separate cards, not one conditionally-reshaped card, whenever a screen has both an always-true status fact and a sometimes-true actionable consequence of that fact — collapsing them loses the "this is always true" signal the plain banner carries once it starts disappearing depending on other state.
- **`pillSuccess` added alongside the existing `pillWarning`/`pillError`** (`OrderDetail.module.css`) — same shape (`color: var(--state-X); background: color-mix(..., transparent 90%); border: 1px solid color-mix(..., 33%, transparent)`), just the success token. The 3-tone family was previously warning/error only; reach for `pillSuccess` for any future pill representing a fully-resolved/good outcome (used for the new item-delivered pill's "fully sent" state) rather than inventing a fourth ad-hoc green treatment.
- **New `owedLinesBox`/`owedLineItem`/`owedLinePill` for a bordered box containing bordered pills** — the outer box (`border` + `radius-md` + `padding`, plain background, `flex-direction: column`) holds one `owedLineItem` per line (name label, then a nested `owedLinePill` — its own border/radius/padding, `width: fit-content` so it doesn't stretch full-width like the outer box). Reach for this nested-box-of-pills shape whenever a card needs to list several named things each with their own small bordered detail, rather than flattening everything into one list or reusing `packRow` (filled-background single row, no nesting) which doesn't fit a two-level "label, then detail chip" shape.
- **A staged/working field converted into a permanent one at the moment of confirmation, not written twice** — `order_lines.sent` (Cold Storage's "how much am I sending this run" input, already live) is read once at `handleConfirmDelivery` and folded into `delivered` (`delivered += sent`, then `sent` cleared), rather than either field being written redundantly at both stages. Reach for this "staged→permanent, cleared on commit" pattern whenever a value is provisional during one stage and needs to become part of a running total once a later stage confirms it — keeps exactly one field meaning "still being decided" and one meaning "settled," never both claiming the same thing at once.
- **A capability-restricted role missing *all* write access to a collection, not just missing fields**: Courier previously had zero `order_lines` update permission (not even a restricted one) — discovered only because a new write path (`delivered`/`sent` at delivery-confirm) needed it for the first time. Different from this project's much more common "field exists but isn't in the role's restricted `fields:[...]` allow-list" gap (documented in `code-standards.md`) — worth checking for the collection-level grant existing at all, not just assuming a present-but-narrow one, when a role's `permissions?filter[role][_eq]=...&filter[collection][_eq]=...` query for an action comes back with an empty `data: []` array rather than a narrow-fields row.

## Update — 2026-08-28 (2) `Button`'s `tone` prop — sentiment color as a dimension orthogonal to `variant`

- **`Button` gained a `tone?: "primary" | "neutral" | "success" | "warning" | "error"` prop** (`src/components/Button/Button.tsx`, `Button.module.css`), independent of the existing `variant` prop. `variant` (`primary`/`secondary`/`tertiary`/`ghost`) controls *structure* — filled, outlined, plain-text, transparent; `tone` controls *color* — which sentiment gets used as the accent within whichever structure `variant` picked. The two compose freely: `variant="secondary" tone="error"` is an outlined red button, `variant="primary" tone="success"` is a solid green one, etc.
- **Implementation pattern — CSS custom properties as the seam between the two dimensions**, reach for this whenever a component needs two independent visual axes (structure × color) without an N×M explosion of classes: each `.tone-*` class sets three custom properties (`--btn-accent`, `--btn-accent-dark`, `--btn-on-accent`) on the button element; the `variant` rules (`.primary`, `.secondary`, `.tertiary`, `.ghost`, `.iconOnly`) reference only those three variables, never a hardcoded palette color directly. Adding a new tone later is a 3-line CSS block, not a touch to every variant rule.
- **`tone` defaults to `"primary"`**, so every existing call site (this Button is used dozens of places across the app) keeps its exact current appearance with zero changes — the default tone's custom properties resolve to the same `--accent-primary`/`--accent-primary-dark`/`--text-on-accent` tokens the variant rules referenced directly before this change.
- **One new token added, `--text-on-warning` (`tokens.css`)**: deliberately defined once in `:root` and NOT overridden in the dark-theme block — both theme's warning ambers (`#f59e0b` light / `#fbbf24` dark) are light/bright colors, so the same dark text value is correct against either one regardless of overall app theme. Reach for this same "theme-invariant token, single definition" pattern whenever a token's correct value doesn't actually depend on which app theme is active — don't reflexively duplicate every token into both theme blocks if the underlying reason for having two blocks (inverted light/dark backgrounds) doesn't apply to that particular value.
