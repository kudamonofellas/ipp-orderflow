...
| `backorder_of`      | UUID, nullable, self-FK → `orders.id`, `ON DELETE SET NULL` | **added 2026-09-02, converted from a plain TEXT `no` string to a real FK same day** — the parent order this backorder was split from, set on the `#{no}-B` child created from the "Part delivered" card's "Create backorder" action. Enforced and navigable (Postgres rejects a non-existent parent id) rather than a loose string match on `no`. |
