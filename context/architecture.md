...
**Installable web app (iPhone/desktop):** `public/manifest.webmanifest` + `public/icons/` + apple meta tags in `index.html`. Icons: `icon-192`/`icon-512` (purpose `any`) have rounded corners (radius 22%, transparent outside); `icon-maskable-512` and `apple-touch-icon` stay square and full-bleed on purpose — Android masks maskable icons to its own shape, and iOS rounds apple-touch icons itself and fills transparency with black. iPhone users install via Safari → Share → Add to Home Screen (an `.apk` can't run on iOS; a native iOS build would need a Mac/Xcode and an Apple Developer account). **No service worker by design** — it would cache old builds past a deploy, defeating nginx's `no-store` on `index.html` and the live-URL APK.

**Android APK:** `npm run build:apk` = prod `npm run build` → `cap sync android` → `gradlew assembleDebug`; output `android/app/build/outputs/apk/debug/app-debug.apk` (debug-signed, for sideloading). Toolchain: Microsoft OpenJDK 21 + Android SDK command-line tools (platform 36, build-tools 36.0.0) in `%LOCALAPPDATA%\Android\Sdk`; `JAVA_HOME`/`ANDROID_HOME` set as user env vars — no Android Studio. **The APK is a thin shell around the live site:** `capacitor.config.ts` sets `server.url: 'https://app.kudafellas.cloud'`, so the WebView loads the deployed web app rather than the bundled copy. A web deploy (`npm run build` + scp) therefore updates phones too; rebuild/reinstall the APK only for native changes (manifest permissions, icon, app name, Capacitor plugins). The origin is the real site, already in prod Directus's `CORS_ORIGIN` (the default bundled origin `https://localhost` is rejected by both instances). The app needs internet to open. Auth is JSON tokens in localStorage, so no cookie concerns. `parseOrderText()` uses the relative proxy URL only when `import.meta.hot` is set (Vite dev server), not on `import.meta.env.DEV`. Manifest declares location + camera permissions for geolocation and photo inputs.
...
- **`order_lines`** — replaces `orders.order_items` text blob. Fields: `order_id` → `orders`, `product_id` → `products` (nullable for free-text), `name` (snapshot), `qty` NUMERIC(12,3), `unit` (kg/gram/pack/pcs/box/ekor/loaf), `weight` NUMERIC(10,3) (actual catch-weight), `price` NUMERIC(15,2) (PO-stated only; NULL = priced in Accurate), `status` (recognized/manual/unmatched), `delivered` INT, `returned` INT, `sent` NUMERIC(12,3) (how much of `qty` is actually going out this round; NULL = full qty), `short` BOOL, `removed` BOOL, `weigh_photo` → `directus_files`, `returned_weigh_photo` → `directus_files`, `inbound_return` NUMERIC(12,3) (snapshot of `returned` taken when a replacement is settled before the goods physically come back — cleared once the warehouse confirms receipt via the Incoming Return card; NULL the rest of the time), `sort_order` INT.
...
4. **Every order has a status from the pipeline enum.** Stable keys: `intake → cold → finance → production → packing → finalise → dispatch → delivered` (current UI labels: `New Orders`, `Cold Storage`, `Finance Review`, `Processing`, `Packing`, `Print DO/SI`, `Dispatch`, `Delivered`). Off-pipeline states: `outstanding`, `awaiting`, `cancelled`, `returned`. Return workflow stages (parallel buckets, not sequential) are tracked separately: `awaiting_return`, `admin_action`, `awaiting_signed_doc`, `replacement_transit` — see `src/lib/pipeline.ts`. The legacy schema's default `'Draft'` maps to `intake`.
   - **`outstanding` has two independent causes that can coexist** (clarified 2026-09-02): a COD payment shortfall (all items delivered, cash owed — Finance's problem) and an item shortfall (some ordered qty never left the warehouse this run — `OrderDetail.tsx`'s `owedLines`, ported from the prototype's own `outstanding` meaning). `handleConfirmDelivery` routes to `outstanding` when either is true; the UI shows the item-shortfall "Part delivered" decision card only when `owedLines.length > 0`, alongside the always-shown plain status banner. `awaiting` is the stage a backorder (`#{no}-B`, `backorder_of`/`remind_on` set) lands at when the item-shortfall remainder is deferred rather than re-sent immediately.
