...
4. **Every order has a status from the pipeline enum.** `intake → cold → finance → production → packing → finalise → dispatch → delivered`, plus off-pipeline: `outstanding`, `awaiting`, `cancelled`, `returned`. The current schema's default `'Draft'` maps to the `intake` stage.
