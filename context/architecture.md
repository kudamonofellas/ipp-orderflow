...
**Installable web app (iPhone/desktop):** `public/manifest.webmanifest` + `public/icons/` + apple meta tags in `index.html`. iPhone users install via Safari → Share → Add to Home Screen (an `.apk` can't run on iOS; a native iOS build would need a Mac/Xcode and an Apple Developer account). **No service worker by design** — it would cache old builds past a deploy, defeating nginx's `no-store` on `index.html` and the live-URL APK.
